...
namespace MSIX {
        ...
        m_container(container)
    {
        ...
        ThrowErrorIfNot(Error::MissingAppxManifestXML, (appxManifestInContainer.second || appxBundleManifestInContainer.second) , 
            "AppxManifest.xml or AppxBundleManifest.xml not in archive!");
        ThrowErrorIf(Error::MissingAppxManifestXML, (appxManifestInContainer.second && appxBundleManifestInContainer.second) , 
        ...
        {
            stream = m_appxBlockMap->GetValidationStream(APPXMANIFEST_XML, appxManifestInContainer.second);
        ...
        else
        {
            ...
            stream = m_appxBlockMap->GetValidationStream(pathInWindows, appxBundleManifestInContainer.second);
        ...
        std::map<std::string, Config> footPrintFileNames = {
            ...
            //{ APPXBUNDLEMANIFEST_XML,  Config([&]()->IStream*{ m_footprintFiles.push_back(APPXBUNDLEMANIFEST_XML);  return nullptr;})  }, // TODO: IMPLEMENT
        };
        ...
        if(m_isBundle)
        {
            ...
            {   auto footPrintFile = footPrintFileNames.find(fileName);
                if (footPrintFile == footPrintFileNames.end())
                {
                    ...
                    ComPtr<IStream> package(m_container->GetFile(fileName).second);
                    m_streams[fileName] = std::move(package);
                    filesToProcess.erase(std::remove(filesToProcess.begin(), filesToProcess.end(), fileName), filesToProcess.end());
                }
            }
        }
        else
        {
            ...
            {   auto footPrintFile = footPrintFileNames.find(fileName);
                ...
                {   std::string containerFileName = EncodeFileName(fileName);
                    ...
                    VerifyFile(fileStream.second, fileName, blockMapInternal);
    ...
    void AppxPackageObject::VerifyFile(IStream* stream /*Will be a const MSIX::ComPtr<IStream>& stream after nip_tuck*/, const std::string& fileName, const ComPtr<IAppxBlockMapInternal>& blockMapInternal)
    {    
