...
namespace MSIX {
    ...
    public:
        ...
        FileStream(const std::string& name, Mode mode) : m_name(name)
        {
            ...
            ThrowErrorIfNot(Error::FileOpen, (err==0), std::string("file: " + m_name + "does not exists.").c_str());
            #else
            m_file = std::fopen(name.c_str(), modes[mode]);
            ThrowErrorIfNot(Error::FileOpen, (m_file), std::string("file: " + m_name + "does not exists.").c_str());
            #endif
