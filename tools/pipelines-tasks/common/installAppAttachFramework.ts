...
async function installNuget(packagePath: string, outputPath: string, targetNetFramework: string, packageId?: string, version?: string) {
    try {
        ...
        console.error("Error finding target framework Dlls: " + error);
        let errorOutput: string = error as string;
        // Define a regular expression to capture the error ID
        const errorIdRegex = /FullyQualifiedErrorId\s*:\s*(.*)/;
        const match = errorOutput.match(errorIdRegex);
        let errorId: string | null = null;

        if (match && match[1]) {
            // Extract the captured error ID
            errorId = match[1];
        }
        throw errorId;
...
async function run(): Promise<void> {
    try {
        ...
        console.error("AppAttachFramework Install Error"+ error);
        throw "new exc";
    }
}
